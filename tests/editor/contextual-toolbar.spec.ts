...
test.describe('Contextual toolbar', () => {
  ...
  test('Filters tab does not show palette-only controls', async ({ page }) => {
    const toolbar = page.getByTestId('contextual-toolbar')
    await expect(toolbar.getByRole('button', { name: 'Sort' })).not.toBeVisible()
    await expect(toolbar.getByRole('button', { name: 'Export' })).not.toBeVisible()
  })

  // --- Palette tab ---

  test('Palette tab shows Sort and Export buttons', async ({ page }) => {

...
describe('activePigmentsFromSettings', () => {
  it('filters by enabledNames in catalog order', () => {
    ...
    const out = activePigmentsFromSettings({ enabledNames: ['C', 'A'], minPaintPercent: 2, deltaThreshold: 4 }, catalog);
    expect(out.map((p) => p.name)).toEqual(['A', 'C']);
  });

  it('defaults catalog to PIGMENTS', () => {
    const first = PIGMENTS[0]!.name;
    const out = activePigmentsFromSettings({ enabledNames: [first], minPaintPercent: 1, deltaThreshold: 1 });

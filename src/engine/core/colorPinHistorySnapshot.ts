 ...
 * (id, image position, radius, color, and optional label/groupId only when defined) so history stacks and drag snapshots
 * do not alias live
...
export function cloneColorPinSnapshot(pins: readonly ColorPin[]): readonly ColorPin[] {
  // Match persistence rules: Firestore `setDoc` rejects nested `undefined`; omit optional keys instead of copying
  // `label` / `groupId` as explicit undefined (see `applySetAllPins` in colorPinMutations).
  return pins.map((p) =>
    Object.freeze({
      id: p.id,
      imageX: p.imageX,

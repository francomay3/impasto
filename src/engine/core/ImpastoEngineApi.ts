...
import type { ColorPinGroup } from '../colorPins/ColorPinGroupState';
...
export type ImpastoEngineToolsApi = {
  ...
  /** No-op when sample-color is not active. Same rules as keyboard brush nudge. */
...
/** Options for {@link ImpastoEngineDocumentApi.loadDocument}. */
export type LoadDocumentOptions = {
  /**
   * - `hydrate` (default): loading from persistence — do **not** notify `subscribeDocumentChanged`, so autosave does
   *    not fire immediately after downloading the same bytes we just loaded.
   * - `user-edit`: durable document was replaced by explicit user action (import image, etc.) — **do** notify so
   *    persistence autosave can run.
   */
  documentChangeIntent?: 'hydrate' | 'user-edit';
};
...
export type ImpastoEngineDocumentApi = {
  ...
  /**
   * Fires when the durable document changes in a way that should be persisted — undo stack mutations, redo, and
   * (when {@link LoadDocumentOptions.documentChangeIntent} is `user-edit`) wholesale `loadDocument` replacements.
   * Persistence glue should use this hook only; the engine defines what counts as a document change.
   */
   ...
   * unless `documentChangeIntent` is `user-edit`.
   * When `sourceImage` is provided it is applied via the internal snapshot path (no defensive copy); omit it to leave
   * the current source image unchanged. Tool and selection are unchanged; viewport may refit when mounted viewports
   * observe a new `RawImage` instance (see {@link useFitOnImageLoad}).
   */
  loadDocument(snapshot: ImpastoDocumentSnapshot, sourceImage?: RawImage | null, options?: LoadDocumentOptions): void;
};
...
export type ImpastoEngineColorPinsApi = {
  ...
  /**
   * Reorders pins to match `orderedIds` (a permutation of current pin ids). Updates indexed palette slot order
   * and records one undo step when the order changes. No-op if the list is empty, lengths differ, ids are missing,
   * or duplicates appear.
   */
  reorderTo(orderedIds: readonly string[]): void;
  /**
  ...
  /** Sets the display label on a pin. Undoable. */
  setPinLabel(id: string, label: string): void;
  /** Assigns a pin to a group, or removes it from its current group when `groupId` is `undefined`. Undoable. */
  setPinGroup(id: string, groupId: string | undefined): void;
};

export type ImpastoEngineColorPinGroupsApi = {
  getAll(): readonly ColorPinGroup[];
  subscribe(listener: () => void): () => void;
  /** Creates a group, returns its id. */
  add(label: string, color?: string): string;
  removeById(id: string): void;
  setLabel(id: string, label: string): void;
  setAllGroups(groups: readonly ColorPinGroup[]): void;
};

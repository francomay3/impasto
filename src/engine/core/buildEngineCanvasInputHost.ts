...
import type {
  ViewportCanvasInputHost,
} from '../viewports/canvas/host/viewportInputPolicy';
...
type MarqueeDragEndPayload = Parameters<NonNullable<ViewportCanvasInputHost['marqueeDragEnd']>>[0];

function createMarqueeDragEndHandler(deps: BuildEngineCanvasInputHostDeps) {
  const { ensureLive, marqueeGesture, selection, toolState, colorPinsState } = deps;

  return (payload: MarqueeDragEndPayload) => {
    ensureLive();
...
export function buildEngineCanvasInputHost(deps: BuildEngineCanvasInputHostDeps): ViewportCanvasInputHost {
  const { toolState, selection, marqueeGesture, colorPins, ensureLive } = deps;
  const marqueeDragEnd = createMarqueeDragEndHandler(deps);

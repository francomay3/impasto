...
export class ImpastoEngine implements ImpastoEngineDocumentApi {
  ...
  constructor() {
    ...
    this.colorPinGroups = b.colorPinGroups;
  ...
  getDocumentSnapshot(): ImpastoDocumentSnapshot {
    ...
    const groups = Object.freeze([...this.colorPinGroups.getAll()]);
  ...
  loadDocument(snapshot: ImpastoDocumentSnapshot, sourceImage?: RawImage | null, options?: LoadDocumentOptions): void {
    ...
    this.colorPinGroups.setAllGroups(snapshot.groups ?? []);
    this._historyManager.clearSilently();
    if ((options?.documentChangeIntent ?? 'hydrate') === 'user-edit') {
      this._historyManager.notifyExternalDocumentChange();
    }
  }

...
import {
  computeMergedPinInsertIndex,
  resolveGroupIdAfterPinMerge,
  type ColorPinGroupOrderRef,
} from './mergeColorPinListPlacement';
...
type ColorPinBlendDeps = {
  ...
  /** Current palette group rows in list order; used only when merging pins (`keepOriginals === false`). */
  readonly getColorPinGroupsOrdered: () => readonly ColorPinGroupOrderRef[];
};
 ...
 * When `keepOriginals` is false (merge), source pins are removed, the new pin is spliced at the rescaled mean engine
 * index ({@link computeMergedPinInsertIndex}), and `groupId` follows {@link resolveGroupIdAfterPinMerge}. When true
 * (add-middle), sources remain and the new pin is appended with no merge `groupId`.
 */
export function blendColorPinsFromIds(
  ...
  let newId: string;
  if (keepOriginals) {
    newId = deps.colorPins.addFromSample(geometry, sampledColorHex);
  } else {
    const mergedEngineIndices = pins.map((p) => before.findIndex((b) => b.id === p.id));
    const insertIndex = computeMergedPinInsertIndex(before.length, mergedEngineIndices);
    const mergedGroupId = resolveGroupIdAfterPinMerge(pins, deps.getColorPinGroupsOrdered());
    deps.colorPins.removeByIds(pins.map((p) => p.id));
    newId = deps.colorPins.addFromSampleInsertAt(
      {
        ...geometry,

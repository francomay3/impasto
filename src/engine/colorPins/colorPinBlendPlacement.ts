...
/** Full raster scan: tracks global-best ΔE and centroid‑closest pixel among those within `maxDeltaE`. */
function scanRasterForBlendedLabCandidates(
  ...
  maxDeltaE: number,
  minAlpha: number,
): { bestConstrained: Cand | null; bestGlobal: Cand | null } {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const centerAlpha = raster.data[(y * raster.width + x) * 4 + 3]!;
      if (centerAlpha <= minAlpha) {
        continue;
      }
      const [sr, sg, sb] = sampleCircleAverage(raster, x, y, 1);
  ...
  return { bestConstrained, bestGlobal };
}
...
export function nearestRasterPixelToCentroidForBlendedLab(
  ...
  const { bestConstrained, bestGlobal } = scanRasterForBlendedLabCandidates(
    raster,
    cx,
    cy,
    targetLab,

...
import type {
  ImpastoEngineColorPinPlacementExtents,
  ImpastoEngineColorPinsApi,
} from '../core/ImpastoEngineApi';
...
export class ColorPinCoordinator {
  ...
  /** Group row order for merge `groupId` resolution; defaults to empty when omitted (tests / legacy callers). */
  private readonly _getColorPinGroupsOrdered: () => readonly { readonly id: string }[];
  ...
  public constructor(
    ...
    ensureLive: () => void,
    getColorPinGroupsOrdered?: () => readonly { readonly id: string }[],
  ) {
    ...
    this._getColorPinGroupsOrdered = getColorPinGroupsOrdered ?? ((): readonly { readonly id: string }[] => []);
  }
  ...
  public buildApi(): ImpastoEngineColorPinsApi {
    return {
      ...
      ...this._colorPinsApiDragBlendAndMeta(),
    };
  }

  /** Pointer-drag session, merge / middle-pin blend, and pin metadata — everything beyond {@link buildColorPinPlacementApi}. */
  private _colorPinsApiDragBlendAndMeta(): Pick<
    ImpastoEngineColorPinsApi,
    | 'beginPointerDrag'
    | 'endPointerDrag'
    | 'abortPointerDrag'
  ...
  > {
    return {
      ...
      setPinLabel: (id, label) => {
        this._mutatePinsWithHistory(() => this._colorPins.setPinMeta(id, { label }));
      },
      setPinGroup: (id, groupId) => {
        this._mutatePinsWithHistory(() => this._colorPins.setPinMeta(id, { groupId: groupId ?? null }));
      },
    };
  }

  /** Single undo step when `mutate` changes the pin list (label / group edits). */
  private _mutatePinsWithHistory(mutate: () => void): void {
    this._ensureLive();
    const before = cloneColorPinSnapshot(this._colorPins.getAll());
    mutate();
    const after = cloneColorPinSnapshot(this._colorPins.getAll());
    this.pushColorPinHistoryIfChanged(before, after);
  }
    ...
    blendColorPinsFromIds(
      {
        ...
        pruneSelection: this._pruneSelection,
        pushColorPinHistoryIfChanged: (before, after) => this.pushColorPinHistoryIfChanged(before, after),
        getColorPinGroupsOrdered: () => this._getColorPinGroupsOrdered(),
      },
      ids,
      keepOriginals,
    );

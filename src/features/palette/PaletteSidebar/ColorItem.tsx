      ...
      <Stack gap={4}>
        ...
        <Box style={{ display: 'flex', alignItems: 'center', gap: 4, paddingLeft: 46 }}>
          <Box style={{ display: 'flex', alignItems: 'center', gap: 4, flex: 1, minWidth: 0 }}>
            {color.name && <Text data-testid="color-hex" ff="monospace" c="dimmed" style={{ fontSize: 10, userSelect: 'text' }}>{color.hex.toLowerCase()}</Text>}
            {color.ratio > 0 && <Badge data-testid="color-ratio" size="xs" variant="outline" color="gray">{color.ratio}%</Badge>}
          </Box>
          <Box style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
            <Menu shadow="md" width={160} position="bottom-end">
              <Menu.Target>
                <Tooltip label="Move to group">
                  <ActionIcon size="sm" variant="subtle" color={color.groupId ? 'blue' : 'gray'} data-testid="color-group-button"><Folder size={13} /></ActionIcon>
                </Tooltip>
              </Menu.Target>
              <Menu.Dropdown>

...
export function useColorContextMenu() {
  ...
  const open = useCallback((colorId: string, pos: { x: number; y: number }, opts: OpenOptions = {}) => {
      ...
      items: [
        ...
        ...(opts.onEditStart ? [{ label: 'Edit', icon: <Settings2 size={14} />, onClick: opts.onEditStart }] : []),
        {
          ...
          onClick: () => {
            ...
            notifications.show({ message: `Copied ${color.hex.toLowerCase()}`, color: 'green', autoClose: 1500 });
          },
        },
        ...(color.sample ? [{ label: 'Remove pin', icon: <PinOff size={14} />, onClick: () => onRemoveSamplePin(colorId) }] : []),
        { type: 'divider' as const },
        { label: 'Delete color', icon: <Trash2 size={14} />, onClick: () => onDeleteColor(colorId), color: 'red' },
      ],
    });

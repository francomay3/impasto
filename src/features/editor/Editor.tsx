...
import {
  ReplaceImageModal,
  type ReplaceImageModalRef,
} from "./ReplaceImageModal";
...
export default function Editor({
  ...
  const { hiddenPinIds, onTogglePinVisibility, onSetGroupPinsVisible } =
    useHiddenPins();
  ...
  const {
    ...
    setImage,
    updateColor,
    setPalette: updateDerivedPalette,
    addSampledColor,
    removeColor,
    updateFilter,
    samplingColorId,
  ...
  const paletteValue = {
    ...
    onSampleColor: handleSampleWithSelect,
    onCancelSampleColor: handleCancelSample,
    ...
    onAddColorAtPosition: handleAddColorAtPosition,
    onDeleteColor: handleDeleteColor,
    onPinMoveEnd: handlePinMoveEnd,
    onRemoveSamplePin: (id: string) => updateColor(id, { sample: undefined }),
    onAddGroup: addGroup,
  ...
  const filterValue = {
    ...
    onPreviewFilter: updateFilterPreview,
  ...
  const canvasValue = {
    ...
    isSampling:
      activeTool === "eyedropper" || !!samplingColorId || !!samplingLevels,
    showLabels,
  ...
  const editorValue = {
    ...
    onSetGroupPinsVisible,
  };

  return (
    <ToolContext.Provider
      value={{ activeTool, setActiveTool, samplingRadius, setSamplingRadius }}
    >
            ...
            <SelectionPopoverProvider>
              <FilterContext.Provider value={filterValue}>
                <AppShell header={{ height: 68 }} padding={0}>
                  ...
                  <ReplaceImageModal
                    ref={replaceRef}
                    hasSamples={state.palette.some((c) => c.sample)}
                    onFileSelected={handleFileSelected}
                  />
                </AppShell>

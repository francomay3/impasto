...
### 1. Memoize `resolveLabelOffsets` in `SamplePinsOverlay`
...
### 2. Memoize context provider value objects
...
### 3. Add `useMemo` to derived arrays in `PaletteSidebar` and `SamplePinsOverlay`
...
### 4. Add `will-change: transform` to canvas wrapper
...
### 5. Throttle `SamplerOverlay` canvas redraw with `requestAnimationFrame`
...
### 6. Throttle palette DnD `handleDragOver` with `requestAnimationFrame`
...
### 7. Move viewport pan/zoom off React state — use CSS transform
...
### 8. Fix `SamplePinsOverlay` `useLayoutEffect` depending on `viewportTransform`
...
### 9. Move filter pipeline to an `OffscreenCanvas` Web Worker
...
### 10. Throttle `sampleCircleAverage` calls during palette sync
...
### 11. Cache and reuse temporary canvases in `blurImageData`
...
### 12. PixiJS for the viewport canvas
...
### 13. WebAssembly for image processing (k-means + pixel ops)
...
### 14. Instant filter preview via WASM + OffscreenCanvas pipeline
...
### 15. SharedArrayBuffer for zero-copy worker communication
...
### 16. WebGPU compute shaders for filter chains (future)
